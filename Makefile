...
COMMON_MAKEROM_FLAGS := -rsf $(RSF_FILE) -target t -exefslogo -icon icon.icn -banner banner.bnr -major $(APP_VERSION_MAJOR) -minor $(APP_VERSION_MINOR) -micro $(APP_VERSION_MICRO) -DAPP_TITLE="$(APP_TITLE)" -DAPP_PRODUCT_CODE="$(APP_PRODUCT_CODE)" -DAPP_UNIQUE_ID="$(APP_UNIQUE_ID)" -DAPP_CATEGORY="$(CATEGORY)" -DAPP_USE_ON_SD="$(USE_ON_SD)" -DAPP_MEMORY_TYPE="$(MEMORY_TYPE)" -DAPP_CPU_SPEED="$(CPU_SPEED)" -DAPP_ENABLE_L2_CACHE="$(ENABLE_L2_CACHE)" -DAPP_VERSION_MAJOR="$(APP_VERSION_MAJOR)"


$(OUTPUT).3ds: stripped.elf banner.bnr icon.icn
	@$(MAKEROM) -f cci -o $(OUTPUT).3ds -elf stripped.elf -DAPP_ENCRYPTED=true $(COMMON_MAKEROM_FLAGS)
	@echo "built ... $(notdir $@)"

$(OUTPUT).cia: stripped.elf banner.bnr icon.icn
	@$(MAKEROM) -f cia -o $(OUTPUT).cia -elf stripped.elf -DAPP_ENCRYPTED=false $(COMMON_MAKEROM_FLAGS)
	@echo "built ... $(notdir $@)"
