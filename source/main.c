...
void* memoryCopy(void* a_pDest, void* a_pSrc, u32 a_uSize)
{
	...
	if (bIsDestWithinGSP)
	{
		if (bIsSrcWithinGSP)
		{
			s64 nRemain = a_uSize;
			s32 nOffset = 0;
			printf("\E[%d;0H""执行GSPWN\n", g_nCurrentLine++);
			printf("\E[%d;0H""从%08x复制到%08x\n", g_nCurrentLine++, (u32)a_pSrc, (u32)a_pDest);
...
u32 changeSharedFont(SharedFontType a_eType)
{
	...
	if (g_uFontAddress == 0)
	{
		...
		{
			fclose(fp);
			printf("\E[%d;0H""内存分配出错\n", g_nCurrentLine++);
			return 4;
		}
		else
		{
			printf("\E[%d;0H""内存分配成功\n", g_nCurrentLine++);
			printf("\E[%d;0H""字体地址 %X\n", g_nCurrentLine++, g_uFontAddress);
...
int main(int argc, char* argv[])
{
	...
	{
		bool uOut;
		Result result = APT_CheckNew3DS(&uOut);
		if (result == 0 && uOut)
		{
			g_bNew3DS = true;
		}
